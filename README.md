...
### Logrotate is a Go package for writing logs to rolling files.
...
To use logrotate with the standard library's log package, just pass it into the SetOutput function when your application starts.
...
log.SetOutput(&logrotate.Logger{
    ...
    Daily:      true, // disabled by default
    // DailyFilename: true, // disabled by default
    MaxBackups: 3,
    MaxAge:     28, //days
...
If Daily is true, the current file is also rotated when the current day changes.
The day boundary uses UTC by default, or local time when LocalTime is true.
MaxSize still applies, so a logger can rotate both at day boundaries and when
the current day's file grows too large.

If DailyFilename is true, the active log file name includes the current date in
the form `name-2006-01-02.ext`. For example, `foo.log` becomes
`foo-2026-06-08.log`. At day boundaries, Logger closes the old dated file and
starts writing to the new day's dated file. MaxSize still applies within each
day, and size-based backups keep the normal timestamped backup format.
